...
export default function TrackScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch history
      const historyRes = await fetch(`${BACKEND_URL}/api/tracking/history?limit=10`, {
      ...
      const summaryRes = await fetch(`${BACKEND_URL}/api/tracking/${period}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (summaryRes.ok) {
        ...
        {summary && (
          <View style={styles.summaryCard}>
            <Text style={styles.sectionTitle}>
              {period === 'daily' ? "Today's" : period === 'weekly' ? 'This Week' : 'This Month'} Summary
            </Text>
            ...
            <View style={styles.summaryRow}>
              <View style={styles.summaryItem}>
                ...
                <Text style={styles.summaryValue}>{formatNumber(summary.total_nutrients?.energy_kcal)}</Text>
                <Text style={styles.summaryLabel}>kcal</Text>
              </View>
              <View style={styles.summaryItem}>
                <Ionicons name="barbell" size={20} color="#00d4ff" />
                <Text style={styles.summaryValue}>{formatNumber(summary.total_nutrients?.protein_g)}</Text>
                <Text style={styles.summaryLabel}>Protein (g)</Text>
              </View>
              <View style={styles.summaryItem}>
                <Ionicons name="leaf" size={20} color="#4ecdc4" />
                <Text style={styles.summaryValue}>{formatNumber(summary.total_nutrients?.carbohydrate_g)}</Text>
                <Text style={styles.summaryLabel}>Carbs (g)</Text>
              </View>
              <View style={styles.summaryItem}>
                <Ionicons name="water" size={20} color="#ffd93d" />
                <Text style={styles.summaryValue}>{formatNumber(summary.total_nutrients?.fat_g)}</Text>
              ...
              <View style={styles.elementsRow}>
                {['C', 'H', 'O', 'N'].map((el) => (
                  <View key={el} style={styles.elementMini}>
                    ...
                    <Text style={styles.elementAmount}>{formatNumber(summary.total_elements?.[el])}g</Text>
                  </View>
                ))}
              </View>
            </View>
            ...
            {period !== 'daily' && (
              <Text style={styles.daysLogged}>{summary.days_logged || 0} days logged</Text>
            )}
          </View>
        )}

        {/* Quick Add Button */}
        <TouchableOpacity style={styles.quickAddButton} onPress={() => router.push('/(tabs)/search')}>
