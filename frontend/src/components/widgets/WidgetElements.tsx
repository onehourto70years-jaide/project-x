...
export default function WidgetElements({ elements, onViewCharts }: Props) {
  ...
  return (
    <View style={s.wrap}>
      <View style={s.header}>
        <Text style={[s.title, { color: theme.text }]}><Ionicons name="flask" size={16} color={theme.accent} /> {t('dash_elemental')}</Text>
        <TouchableOpacity onPress={onViewCharts} accessibilityRole="link" accessibilityLabel={t('dash_see_charts')}>
          <Text style={[s.seeAll, { color: theme.accent }]}>{t('dash_see_charts')}</Text>
        </TouchableOpacity>
      </View>
