...
ANNUAL_PRICE_EUR = 29.99  # ~2 months free vs monthly
...
async def get_payment_status(user: User = Depends(require_user)):
    ...
    subscription_plan = user_doc.get("subscription_plan", None)  # "monthly" or "annual"
    ...
    # If user has a subscription, check its status with Stripe
        ...
        try:
            ...
            if sub.status not in ("active", "trialing"):
                ...
                await db.users.update_one(
                    {"user_id": user.user_id},
                    {"$set": {"is_premium": False, "subscription_status": sub.status}}
                )
                is_premium = False
...
async def create_checkout(request: Request, user: User = Depends(require_user)):
    ...
    await db.payment_transactions.insert_one({
        ...
        "email": user.email,
        "plan": plan,
        "amount": ANNUAL_PRICE_EUR if plan == "annual" else MONTHLY_PRICE_EUR,
        "currency": "eur",
...
async def stripe_webhook(request: Request):
    ...
    try:
        # Process the event directly (no webhook secret verification for now)
        ...
        if event_type == "checkout.session.completed":
            ...
            if sub_id:
                ...
                logger.info(f"Webhook: Invoice paid for subscription {sub_id}")
            ...
            if sub_id:
                ...
                logger.info(f"Webhook: Invoice failed for subscription {sub_id}")
